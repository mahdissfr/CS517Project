...
def parse_label_thresholds(
    ...
    if not label_threshold_arg:
        raise ValueError("--lael-thresholds was provided but is empty.")
...
def main() -> None:
    ...
    # Read binary_sequence as a string so leading zeros are not lost.
    df = pd.read_csv(
        summary_path,
        dtype={"schedule_id": str, "binary_sequence": str},
    )
    ...
    if args.find_min_budget:
        ...
        result["num_solver_calls"] = (
            int(result["minimum_budget"]) + 1
            if result.get("minimum_budget") is not None
            else len(schedules) + 1
        )
    else:
        result = solve_with_pysmt(
            schedules=schedules,
    ...
    num_coverage_incidences = sum(
        len(schedule_ids)
        for schedule_ids in requirement_to_schedules.values()
    )

    result["instance_statistics"] = {
        "num_csv_rows": int(len(df)),
        "num_schedules": int(num_schedules),
        "num_requirements": int(num_requirements),
        "num_coverage_incidences": int(num_coverage_incidences),
        "coverage_density": (
            num_coverage_incidences / (num_schedules * num_requirements)
            if num_schedules > 0 and num_requirements > 0
            else 0.0
        ),
        "num_initial_groups": int(len(groups)),
        "num_depths": int(len(depths)),
        "num_deadlines": int(len(deadlines)),
        "time_horizon": int(max(times)),
    }

    result["timing_seconds"] = {
        "preprocess_time": preprocess_end - total_start,
