...
# Use exactly the horizon files you showed in your screenshot.
# The older/long CSV is intentionally not included in the runtime-scaling table.
CSV_FILES=(
  "$DATA_DIR/controller_level_summary_T_4.csv"
  "$DATA_DIR/controller_level_summary_T_6.csv"
  "$DATA_DIR/controller_level_summary_T_8.csv"
  "$DATA_DIR/controller_level_summary_T_10.csv"
)

missing=0
for csv in "${CSV_FILES[@]}"; do
  if [[ ! -f "$csv" ]]; then
    echo "ERROR: Missing expected CSV file: $csv" >&2
    missing=1
  fi
done
if [[ "$missing" -ne 0 ]]; then
  echo "Make sure the script is in the same folder as your CSV files, or run with DATA_DIR=/path/to/csv/folder." >&2
  exit 1
fi
